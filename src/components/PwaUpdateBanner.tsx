...
export function PwaUpdateBanner(): React.JSX.Element | null {
  ...
  useEffect(() => {
    const listener = (event: Event): void => setApplyUpdate(() => (event as CustomEvent<ApplyPwaUpdate>).detail);
  ...
  const update = async (): Promise<void> => {
    setIsUpdating(true);
    try { await applyUpdate(); } finally { setIsUpdating(false); }
  };

  return <aside className="update-banner" role="status"><div><strong>新しいバージョンがあります</strong><p>保存データはそのまま、アプリだけを更新します。</p></div><button type="button" disabled={isUpdating} onClick={() => void update()}>{isUpdating ? '更新中…' : '更新'}</button></aside>;
}
