  ...
  return (
    <div className="page-stack">
      <section className="hero-card">
        ...
        <h2>My家計簿へようこそ</h2>
        <p>家計簿データをこの端末のIndexedDBへ保存する基盤を準備しました。</p>
      </section>
      <section className="info-grid">
        <article>
          <small>保存先</small>
          <strong>このiPhone</strong>
          <p>外部サーバーへ家計簿データを送信しません。</p>
        </article>
        <article>
          <small>利用方法</small>
          <strong>PWA</strong>
          <p>Safariからホーム画面へ追加して利用できます。</p>
        </article>
      </section>
    </div>
  );
  ...
  return (
    <div className="page-stack">
      ...
      <section className="notice-card">
        <h2>データ保存について</h2>
        <p>ブラウザのWebサイトデータを削除すると、保存した家計簿データも消える可能性があります。</p>
      </section>
      <section className="notice-card">
        <h2>旧版データ</h2>
        <p>
          以前の画面でLocalStorageに保存した記録は削除せず、確認・書き出し用の旧版画面を残しています。
        </p>
...
function pageFor(tab: AppTabId): React.JSX.Element {
  ...
  if (tab === 'transactions') {
    return <EmptyPage title="収支一覧" text="収支データはまだありません。" />;
  }
  if (tab === 'register') {
    return <EmptyPage title="登録" text="収入・支出の登録機能はPhase 3で実装します。" />;
  }
